...
typedef int (*reader_new_t)(struct nmtk_client_context *client,
		struct nmtk_file_read_context *file, off_t start_offset);
...
struct media_track_slot *media_track_set_param(struct media_track *track,
		media_param_t param, void *value, int value_len);
static inline
struct media_track_slot *media_track_set_param_num(struct media_track *track,
		media_param_t param, int value)
{
	return media_track_set_param(track, param, &value, sizeof(value));
}
...
int mp3_reader_new(struct nmtk_client_context *client,
		struct nmtk_file_read_context *file, off_t start_offset);
...
struct media_parser *rawaudio_parser_new(struct media_track *track,
		struct frameperiod *ts_rate);
int rawaudio_reader_new(struct nmtk_client_context *client,
		struct nmtk_file_read_context *file, off_t start_offset,
		rawaudio_subformat_t subformat, int rate, int channels);
int rawaudio_get_sample_size(rawaudio_subformat_t subformat);
media_format_t waveformat_new_parser(unsigned char *hdr, int len,
