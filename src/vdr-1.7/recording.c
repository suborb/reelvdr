...
#if REELVDR
      if (e->d_name[0] != '.' || (e->d_name[1] == '.' && e->d_name[2] == '.')) { // Ignore all hidden directories and files, except the ones starting with "..."
#else
        if (strcmp(e->d_name, ".") && strcmp(e->d_name, "..")) {
#endif /*REELVDR*/
           char *buffer = strdup(AddDirectory(DirName, e->d_name));
           struct stat st;
#if REELVDR // bug fix; use lstat() to find symlinks
...
#if REELVDR // bug fix; avoid memory leak
                    free(buffer);
#endif
                    continue;
                    }
