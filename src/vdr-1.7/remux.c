...
isyslog("----------------remux SI:ServiceMoveDescriptorTag");
break;
#ifdef USE_TTXTSUBS
...
#define INSERT_EOS_NAL(x) {                                           \
  if(cDevice::PrimaryDevice()->NeedTSCheckEOS() && ((x&0xF0)==0xE0)) {\
    int NewSize = KILOBYTE(1);                                        \
    if(NewSize>size) {                                                \
...
{
#if 0 //def REELVDR
  if(cDevice::PrimaryDevice()->NeedTSCheck()) {
    if ((Length < TS_SIZE) || (Data[0] != TS_SYNC_BYTE) || TsError(Data) || TsIsScrambled(Data)) {
      if(!TsHasPayload(Data)) return; // Don´t reset if it contains no data
      if(length) isyslog("TS packet %s pid %d 0x%04x", TsIsScrambled(Data) ? "SCRAMBLED" : "ERROR", TsPid(Data), TsPid(Data));
      uchar stream = (length && (length>3) && (data[0]==0) && (data[1]==0) && (data[2]==1)) ? data[3] : 0;
      Reset();
      INSERT_EOS_NAL(stream);
      return;
    } // if
    if(TsHasPayload(Data)) {
      if((cont >= 0) && (TsContinuityCounter(Data) != ((cont+1)&TS_CONT_CNT_MASK))) {
        if(length) isyslog("TS packet DISCONTINUE detected pid %d 0x%04x", TsPid(Data), TsPid(Data));
        uchar stream = (length && (length>3) && (data[0]==0) && (data[1]==0) && (data[2]==1)) ? data[3] : 0;
        Reset();
        INSERT_EOS_NAL(stream);
        return;
      } // if
      cont = TsContinuityCounter(Data);
    } else
      return;
  } else {
    if (!TsHasPayload(Data))
...
#ifdef REELVDR
     int NewSize = max(size + KILOBYTE(32), length + Length);
#else
...
#if 0 //RC: obsolete since 1.7.21? TODO: check if really is. def REELVDR
                    // We need to have at least numPtsValues=1 or second qsort will crash
                    // and at least numPtsValues=2 to get a delta...
                    if ((numPtsValues < MaxPtsValues) && ((numIFrames < 2) || (numPtsValues < 2))) { // collect a sequence containing at least two I-frames
#else
