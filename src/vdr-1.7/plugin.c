...
#ifdef USE_PLUGINMISSING
      {
         esyslog("%s:%d loading plugin FAILED; removing it from dll list", __FILE__, __LINE__);
  ...
  for (cDll *dll = dlls.First(); dll; dll = dlls.Next(dll)) {
      ...
      if (p) {
...
#ifdef REELVDR
         if (!p->Initialize()) {
               esyslog("%s:%d initializing plugin %s FAILED; removing it from dll list", __FILE__, __LINE__, p->Name());
               cDll *prev = dlls.Prev(dll);
               dlls.Del(dll, true);
               dll = prev;
               continue;
         ...
         if (!p->Initialize())
          ...
          esyslog("Error: dll has no plugin to initialize!");
      }
  return true;
}
