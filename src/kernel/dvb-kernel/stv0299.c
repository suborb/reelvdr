...
static int stv0299_init (struct dvb_frontend* fe)
{
	...
	stv0299_writeregI(state, 0x02, 0x00); // Power on
...
static int stv0299_read_signal_strength(struct dvb_frontend* fe, u16* strength)
{
	...
	// GA: Combine with AGC1, which gives more useful values
	// when the signal is strong enough (AGC1>-128)
	// Move 0-reference to noise level
		
	signal1=((stv0299_readreg (state, 0x16)+128)<<8)&0xffff;
	signal=(signal+signal1-0x5000);
				    ...
				    struct i2c_adapter* i2c)
{
	...
	stv0299_writeregI(state, 0x02, 0x80); // back into standby
