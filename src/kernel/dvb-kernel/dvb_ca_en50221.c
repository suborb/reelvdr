 ...
 */
static int dvb_ca_en50221_link_init(struct dvb_ca_private *ca, int slot, int do_reset)
{
	int ret;
	int buf_size;
	u8 buf[2];
	...
	// GA: Reset on demand (Freecam needs it...)
	if (do_reset)
                if ((ret = ca->pub->write_cam_control(ca->pub, slot, CTRLIF_COMMAND, CMDREG_RS)) != 0) 
			return ret;
...
static int dvb_ca_en50221_thread(void *data)
{
	...
	while (!ca->exit) {
		...
		for (slot = 0; slot < ca->slot_count; slot++) {
			...
			case DVB_CA_SLOTSTATE_VALIDATE:
				// GA: put reset first instead at #1
			...
			case DVB_CA_SLOTSTATE_WAITFR:
				if (time_after(jiffies, ca->slot_info[slot].timeout)) {
					printk("dvb_ca adapter %d: DVB CAM did not respond in WAITFR :(\n",
					       ca->dvbdev->adapter->num);
					ca->slot_info[slot].slot_state = DVB_CA_SLOTSTATE_INVALID;
					dvb_ca_en50221_thread_update_delay(ca);
					break;
			...
			case DVB_CA_SLOTSTATE_LINKINIT:
				...
				// GA: Init without reset
				if (dvb_ca_en50221_link_init(ca, slot, 0 ) != 0) {
					 // Now with Reset after a short delay
                                        msleep_interruptible(100);
                                        if (dvb_ca_en50221_link_init(ca, slot, 1) != 0) {
				      ...
				      unsigned int cmd, void *parg)
{
	...
	int err = 0;
	int slot;
	unsigned long arg = (unsigned long) parg;  // GA

	dprintk("%s\n", __FUNCTION__);

	switch (cmd) {
	case CA_RESET:
		printk("IOCTL RESET %i\n",(int)arg);
		for (slot = 0; slot < ca->slot_count; slot++) {
//			if (ca->slot_info[slot].slot_state != DVB_CA_SLOTSTATE_NONE) {
			if (arg&(1<<slot)) {  // GA: Allow single slot reset
				dvb_ca_en50221_slot_shutdown(ca, slot);
				if (ca->flags & DVB_CA_EN50221_FLAG_IRQ_CAMCHANGE)
...
#ifdef FOURCHANNELS
		struct reelfpga *fpga = dvbdev->adapter->priv->rfpga;
		if(fpga){
   	        	int slot = (pid->index+1)%4;
	        	*((volatile unsigned char __force *)fpga->regbase+0x580+0x80)=slot;
                	*((volatile unsigned int __force *)fpga->regbase+0x580+0x40+pid->index*4)=pid->pid;
		}
#endif
		break;	
	}
	...
	case CA_SET_DESCR: {
		...
		if (descr->parity > 1)
			return -EINVAL;
                ...
                struct reelfpga *fpga = ((struct reel_unit*)dvbdev->adapter->priv)->rfpga;
		
		if(fpga){
#ifdef FOURCHANNELS
			int slot = (descr->index+1)%4;
			*((volatile unsigned char __force *)fpga->regbase+0x580+0x80)=slot;
#endif
	        	for(n=0;n<8;n++)
		      		*((volatile unsigned char __force *) fpga->regbase + 0x580 + (n + 8 * descr->parity) *4) = descr->cw[n];
		}
		break;	
	}
