...
#define TDA10046H_TIME_ERROR      0x0a
...
#define TDA10046H_FREQ_ERROR_MSB  0x18
#define TDA10046H_FREQ_ERROR_LSB  0x19
#define TDA1004X_CONFC2C3	 0x1b
#define TDA10045H_S_AGC		 0x1a
#define TDA10046H_AGC_IF_LEVEL	 0x1a
...
#define TDA10046H_CHANNEL_INFO2	 0x51
#define TDA10046H_AGC_TUN_LEVEL	 0x52
...
static int tda10046_init(struct dvb_frontend* fe)
{
	...
	// tda setup
	tda1004x_write_mask(state, TDA1004X_CONFC4, 0x20, 0); // disable DSP watchdog timer
	tda1004x_write_byteI(state, TDA1004X_AUTO, 0x07+0x10);    // 50 ppm crystal, select HP stream, Auto offset
	tda1004x_write_byteI(state, TDA1004X_CONFC1, 0x88);      // enable pulse killer
	tda1004x_write_byteI(state, TDA1004X_CONFC2C3, 0x00);      
			   ...
			   struct dvb_frontend_parameters *fe_params)
{
	...
	dprintk("%s %i %i %i %i %i\n", __FUNCTION__,fe_params->frequency/1000,
	       fe_params->u.ofdm.code_rate_HP,fe_params->u.ofdm.guard_interval,
	       fe_params->u.ofdm.transmission_mode,  fe_params->u.ofdm.bandwidth
	       );

	if (state->demod_type == TDA1004X_DEMOD_TDA10046) {
		// setup auto offset
		tda1004x_write_mask(state, TDA1004X_AUTO, 0x10, 0x00);
		tda1004x_write_mask(state, TDA1004X_IN_CONF1, 0x80, 0x00);
		tda1004x_write_mask(state, TDA1004X_IN_CONF2, 0xC0, 0x00);
...
//	if (state->demod_type == TDA1004X_DEMOD_TDA10045)  // The 10046 also...
	{
...
#if 0
	{
		// Return auto offset findings
		int oc1,oc2,off;
		oc1=tda1004x_read_byte(state, 0x4)&0x80;
		oc2=tda1004x_read_byte(state, 0x5)&0xc0;
		off=(oc1>>5)|(oc2>>6);
		if (off!=0)
			*fe_status |=(off<<8)|(1<<16);
	}
	printk("%s: fe_status=0x%x\n", __FUNCTION__, *fe_status);
#endif
