...
//    hexdump(vfi.data,32);
    ...
    if (vfi.vformat!=0 && (vfi.generation != pl->video_generation || vfi.vformat != pl->last_vformat))
    {
	    ...
	    if (pl->dvbp && pl->dvbp->active) {
		    printf("Close DVB player\n");
		    del_dvb_player(pl->dvbp);
		    pl->dvbp=NULL;
	    }
      
	    if (pl->vplay->fd<0) {
		    printf("open video player\n");
		    open_video_player(pl->vplay);
	    }
...
//	ts_demux_set_pids(pl->td, vpid, apid);
	...
	if (vpid != old_vpid)
        ...
        if (pl->vplay->fd>0) {
          printf("Close video player\n");
          close_video_player(pl->vplay);
          usleep(300*1000);
        }
        if (pl->aplay->fd>0) {
          close_audio_player(pl->aplay);
          usleep(300*1000);
        }
                
        if (!pl->dvbp) {
          pl->dvbp=new_dvb_player(0);
        }
	pl->video_generation=packet->generation;
	if (pl->trickmode)
    	  dvb_set_ts_data(pl->dvbp, vpid, 0, 25);
        else      
          dvb_set_ts_data(pl->dvbp, vpid, apid, 25);
	pl->td->synced = 0;
    }
    if(data_size>0){
	    //printf("DEBUG: put_packet_ts_data: len: %i\n", data_size);
//	    ts_parse(pl->td,data,data_size);
	    dvb_write_ts(pl->dvbp, data, kmem+sizeof(hd_packet_ts_data_t), data_size );
    }
}
/*-------------------------------------------------------------------------*/
static void put_packet_af_data(player_t *pl, hd_packet_af_data_t const *packet)
...
static void put_packet_es_data(player_t *pl, hd_packet_es_data_t const *packet)
{
	...
	printf("%i %i\n",pl->video_generation,packet->generation);
	if (pl->video_generation!=0xaa) {
		  dvb_set_es_data(pl->dvbp, 0,0);
		  pl->video_generation=0xaa;
	}
#endif
...
static void put_packet_es_data_mpa(player_t *pl, hd_packet_es_data_t const *packet)
{
//	dvb_write_es(pl->dvbp, packet->timestamp, (uint8_t const *)packet + sizeof(hd_packet_es_data_t),
...
static void packet_trickmode(player_t *pl, hd_packet_trickmode_t const *packet)
{
    ...
    dvb_trick(pl->dvbp, packet->trickspeed?1:0);
}
/*-------------------------------------------------------------------------*/
