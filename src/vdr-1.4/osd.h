...
#include <limits.h>   //1.6
#include <stdio.h>
#include <stdint.h>
#include "config.h"  //1.6
#include "font.h"
#include "tools.h"  //1.6

#define OSD_LEVEL_DEFAULT     0  //1.6
#define OSD_LEVEL_SUBTITLES  10  //1.6
...
public:
  ...
  inline void UpdateDirty(int x1, int y1, int x2, int y2)
  { 
  ...
  inline tColor GetColor(int x, int y) const { return Color(*Data(x, y)); }
...
//  static int isOpen;
  ...
  bool active; //1.6
protected:
  cOsd(int Left, int Top, uint Level); // 1.6
...
public:
  virtual ~cOsd();
       ///< Shuts down the OSD.
  //static int IsOpen(void) { return isOpen; } // 1.6
  // vdr 1.6 starts ---------
  ...
  virtual void DrawImage(u_int imageId, int x, int y, bool blend, int horRepeat = 1, int vertRepeat = 1); // GT: True color support.
       ...
       //
  virtual void DrawCropImage(u_int imageId, int x, int y, int x0, int y0, int x1, int y1, bool blend){}; //TB
       ///< Puts a 32-Bit true color image at coordinates (x,y), but only draws pixels 
       ///< within the rectangle defined by (x0,y0) and (x1,y1)
  virtual void DrawBitmap32(int x, int y, const cBitmap &bitmap, tColor colorFg, tColor colorBg, bool replacePalette, bool overlay, int width, int height){}; //TB
...
protected:
  virtual cOsd *CreateOsd(int Left, int Top, uint Level) = 0; //1.6
...
public:
  ...
  static cOsd *NewOsd(int Left, int Top, uint Level = OSD_LEVEL_DEFAULT); // 1.6
      ///< Returns a pointer to a newly created cOsd object, which will be located
