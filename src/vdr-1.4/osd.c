...
static tColor lastClrFg[MAX_BLEND_LEVELS], lastClrBg[MAX_BLEND_LEVELS], lastRes[MAX_BLEND_LEVELS];
...
tColor cPalette::Blend(tColor ColorFg, tColor ColorBg, uint8_t Level)
{
  ...
  if(ColorFg == lastClrFg[Level] && ColorBg == lastClrBg[Level]) {
        return lastRes[Level];
  } else {
        lastClrFg[Level] = ColorFg;
        lastClrBg[Level] = ColorBg;
  }
  ...
  lastRes[Level] = (A << 24) | (R << 16) | (G << 8) | B;
  return lastRes[Level];
}

static tColor lastClosestColor;
static tColor lastColor;
static int lastMaxDiff;

int cPalette::ClosestColor(tColor Color, int MaxDiff) const
{
  if(lastColor == Color && lastMaxDiff == MaxDiff) {
     return lastClosestColor;
  }
  int n = 0;
  int d = INT_MAX;
  ...
  lastMaxDiff = MaxDiff;
  lastColor = Color;
  lastClosestColor = d <= MaxDiff ? n : -1;
  return lastClosestColor;
}
...
{
  //printf("BMP::DrawText: x: %i y: %i Text: %s w: %i h: %i font: %p bmp: %p\n", x, y, s, Width, Height, Font, bitmap);
  if (bitmap) {
     int w = Font->Width(s);
     int h = Font->Height();
     int limit = 0;
...
{
    return CreateOsd(Left, Top, Level); // Level 1.6
}
...
void cTextScroller::DrawText(void)
{
  if (osd) {
     for (int i = 0; i < shown; i++) {
         osd->DrawRectangle(left, top + i * font->Height(), left + width, top + (i+1)*font->Height(), colorBg); // TB: clear BG
         osd->DrawText(left, top + i * font->Height(), textWrapper.GetLine(offset + i), colorFg, clrTransparent, font, width);
         }
     }
}
