...
int cDvbSubtitleConverter::Convert(const uchar *Data, int Length)
{
  if (Data && Length > 8) {
     ...
     int SubstreamHeaderLength = 0;
     if(Data[PayloadOffset] == 0x20 && Data[PayloadOffset+1] == 0x01 && Data[PayloadOffset+2] == 0x00) {
         SubstreamHeaderLength = 4;
     }
     bool ResetSubtitleAssembler = (Data[PayloadOffset + 3] != 0x01);
...
void cDvbSubtitleConverter::Action(void)
{
  ...
  while (Running()) {
        ...
        if (cDvbSubtitleBitmaps *sb = bitmaps->First()) {
           ...
           if (1 || Delta <= MAXDELTA) { /* TB: no synchronisation yet */
              if (1 || Delta <= 0) { /* TB: no synchronisation yet */
                 dbgconverter("Got %d bitmaps, showing #%d\n", bitmaps->Count(), sb->Index() + 1);
                 //TB if (AssertOsd()) {
                 /* TODO: fix this! */
                 if (AssertOsd() && sb->Timeout() != 5) {
                    sb->Draw(osd);
