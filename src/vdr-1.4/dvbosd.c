...
public:
  cDvbOsd(int Left, int Top, int OsdDev, uint Level); // 1.6
  virtual ~cDvbOsd();
...
eOsdError cDvbOsd::SetAreas(const tArea *Areas, int NumAreas) // 1.6
{
  if (shown) {
    cBitmap *Bitmap;
    for (int i = 0; (Bitmap = GetBitmap(i)) != NULL; i++) {
...
cOsd *cDvbOsdProvider::CreateOsd(int Left, int Top, uint Level) // 1.6
{
