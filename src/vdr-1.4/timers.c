...
cTimer::cTimer(bool Instant, bool Pause, cChannel *Channel)
{
  ...
  lastSetEvent = 0;
  recorded = recording = pending = inVpsMargin = false;
  flags = tfNone;
  if (Instant)
...
{
  return IsSingleEvent() && !Recording() && StopTime() + EXPIRELATENCY <= time(NULL) && (!HasFlags(tfVps) || !event || !event->Vps() || recorded);
}
...
{
  //printf("\033[0;45m %s(%i): %s %s \033[0m\n", __FILE__, __LINE__, __PRETTY_FUNCTION__, Recording?"TRUE":"FALSE");
...
void cTimer::Skip(void)
{
  ...
  startTime = 0;
  //SetEvent(NULL); //TB: patch rev14307 
  // Reverted: Will crash without if deleting a currently recording timer
  SetEvent(NULL);
}

void cTimer::OnOff(void)
{
  //printf("\033[0;45m %s(%i): %s \033[0m\n", __FILE__, __LINE__, __PRETTY_FUNCTION__);
  if (IsSingleEvent())
     InvFlags(tfActive);
  else if (day) {
     day = 0;
