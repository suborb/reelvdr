...
///< \return Returns a pointer to a newly allocated string.
{
  ...
  strreplace(s, 0x8A, '\n');
  /* TB: convert the string to UTF8 */
  cCharSetConv conv = cCharSetConv("ISO-8859-1", "UTF-8");
  char *s2 = MALLOC(char, Length*2);
  s2[0] = '\0';
  conv.Convert(s, s2, Length*2);
  free(s);
  return s2;
}
...
#if defined(REELVDR) && !defined(RBLITE)
#define CAM_READ_TIMEOUT  3500 // ms
#else
#define CAM_READ_TIMEOUT  10000 // ms
#endif
...
private:
  ...
  int length;
  int esInfoLengthPos;
  uint8_t capmt[2048*2]; ///< XXX is there a specified maximum?
  int caDescriptorsLength;
  uint8_t caDescriptors[2048*2];
  bool streamFlag;
...
public:
  ...
  void AddCaDescriptors(int Length, const uint8_t *Data);
  void AddPid(int Pid, uint8_t StreamType, unsigned short MagicCaID);
  };
...
void cCiCaPmt::AddPid(int Pid, uint8_t StreamType, unsigned short MagicCaId)
{
  magicCaId = MagicCaId;
  if (Pid) {
...
{
  if (esInfoLengthPos) {
     if (length + Length + 6 < int(sizeof(capmt))) {
        capmt[length++] = cmdId;
	/* TB: HACK */
	if(magicCaId > 0 && magicCaId < 3){
           u_int32_t tmpvar = 0x0409;	
           memcpy(capmt + length, &tmpvar, 2);
	   tmpvar = 0x0;
           memcpy(capmt + length + 2, &tmpvar, 1);
	   tmpvar = magicCaId;
	   memcpy(capmt + length + 3, &tmpvar, 1);
           tmpvar = 0x0;
           memcpy(capmt + length + 4, &tmpvar, 1);
	   tmpvar = magicCaId;
           memcpy(capmt + length + 5, &tmpvar, 1);
           memcpy(capmt + length + 6, Data, Length);
           length += 6;
	   //magicCaId = 0; // do this only once
        } else
...
#else
                          PRINTF("###### ADD PID %i %x SLOT %i MAGIC %i\n",q->pid,q->pid, Slot, MagicCaId);
			  if(1) //first)
                            CaPmt->AddPid(q->pid, q->streamType, MagicCaId);
			  else
#endif
                            CaPmt->AddPid(q->pid, q->streamType, 0);
			  first = false;
                          Active = true;
                          }
                       }
