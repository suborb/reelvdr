...
#define REC_UPDATE_FREQ 600 // 10 mins
...
{
  if (ownEvent) {
     if(ownEvent->Components()) ownEvent->Components()->Clear(); //TB: reset the audio languages
     cReadLine ReadLine;
     char *s;
     int line = 0;
...
char *OldExchangeChars(char *s, bool ToFileSystem) // from vdr 1.4.7, used only by extrecmenu to check for existance of directories/files
{
  char *p = s;
  while (*p) {
        if (VfatFileSystem) {
           ...
           if (ToFileSystem) {
              switch (*p) {
                     ...
                     case 'a' ... 'z':
                     case 'A' ... 'Z':
                     case 'ä': case 'Ä': // iso 8859-1 single byte characters
                     case 'ö': case 'Ö': // donot use vi to edit these
                     case 'ü': case 'Ü':
                     case 'ß':
                          break;
                     // characters that can be mapped to other characters:
...
cRecording::cRecording(const char *FileName, const char* BaseDir)
{
  ...
  baseDir = BaseDir?strdup(BaseDir):strdup(VideoDirectory);
...
#if 1
     UpdateInfo();
#else // this block is now in UpdateInfo()
...
bool cRecording::UpdateInfo(bool force)
{
    // if (!force)
    // check info.vdr creation time, if older than last-ScanVideoDir-run 
    // then return false
...
{
  deleted = Deleted;
  lastUpdate = lastModifiedAtLastUpdate = 0;
  state = 0;
  stopScan = false;
...
void cRecordings::Action(void)
{
  bool firstCall = true;
  while (Running()) {
      if ((!deleted && firstCall) || (!deleted && Recordings.NeedsUpdate())) {
           firstCall = false;
           StopScan();
...
void cRecordings::Refresh(bool Foreground)
{
  ...
  Lock();
  //Clear();
  for(cRecording *r = First(); r ; r = Next(r))
    r->UnsetInHdd();

  ChangeState();
  Unlock();
  dsyslog("Starting ScanVideoDir() current Nr. of recordings: %i\n", Count());
  //printf("Starting ScanVideoDir() current Nr. of recordings: %i\n", Count());

  stopScan = false;
  ScanVideoDir(VideoDirectory, Foreground);
  // remove all unseen / unmarked recordings
  Lock();

  // collect all unseen recordings
  cVector<cRecording*> unseen_vec;
  for(cRecording *r = First(); r ; r = Next(r))
    if ( !r->InHdd() ) { unseen_vec.Append(r); /* PRINTF("Removing from recordings list \033[0;91m '%s'\033[0m\n", r->Name()); */ }

  // delete all unseen recordings from cRecordings list
  for (int i = 0 ; i < unseen_vec.Size(); ++i) 
    Del(unseen_vec.At(i));
  //printf("Removed \033[1;91m %i \033[0m\n", unseen_vec.Size());
  ChangeState();
  Unlock();
}
...
void cRecordings::ScanVideoDir(const char *DirName, bool Foreground, int LinkLevel)
{
  ...
  if (DirName == NULL || DVDPlayerActive || stopScan)
      return;
  ...
  cReadDir d(DirName);
  struct dirent *e;
  while ((Foreground || Running()) && (e = d.Next()) != NULL && !stopScan) {
        if (strcmp(e->d_name, ".") && strcmp(e->d_name, "..")) {
           char *buffer = strdup(AddDirectory(DirName, e->d_name));

           // collect all .update files, TODO: check if .update is a regular file ?
           if (!strcasecmp(e->d_name, ".update"))
           {
               cMutexLock MutexLock(&mutexdotUpdates);
               if(dotUpdates.Find(buffer)==-1)
               {
                   dotUpdates.Append(strdup(buffer));
                   //dsyslog(" ------------- .update:= '%s' FOUND, adding to list -------------", buffer);
               }
               //else
                   //dsyslog(" ------------- .update:= '%s' *already* in list -------------", buffer);
           }
           ...
           if (stat(buffer, &st) == 0) {
              ...
              if (S_ISDIR(st.st_mode)) {
                 if (endswith(buffer, deleted ? DELEXT : RECEXT)) {
                     cRecording *oldrec = GetByName(buffer);
                     if (oldrec)
                     {
                        oldrec->UpdateInfo();
                        oldrec->SetInHdd();
                        oldrec->ResetResume(); // does not change resume.vdr, only reset 'resume' var in cRecording so next GetResume() reads resume.vdr
                     }
                     else // recording not present
                     {
...
bool cRecordings::NeedsUpdate(void)
{
  ...
  time_t lastModified1 = 0;

  // search through the list of .update files that were 
  // found under the videoDirectory
  cMutexLock MutexLock(&mutexdotUpdates);
  for (int i=0; i < dotUpdates.Size(); ++i)
  {
      lastModified1 = LastModifiedTime(dotUpdates[i]);
  ...
  bool res = (lastModifiedAtLastUpdate < lastModified) || (!Running() && time(NULL) - lastUpdate > REC_UPDATE_FREQ); // update every 10 mins
  if (res)
    lastModifiedAtLastUpdate = lastModified;
  return res;
}
...
static cMutex cRecordingUserCommandMutex;
static int cRecordingUserCommandActiveCount=0;

void cRecordingUserCommand::InvokeCommand(const char *State, const char *RecordingFileName)
{
    if (command) {
	cMutexLock lock(&cRecordingUserCommandMutex);
	if(!strcmp(RUC_BEFORERECORDING, State))
	    cRecordingUserCommandActiveCount++;
	else if(!strcmp(RUC_AFTERRECORDING, State))
	    if(cRecordingUserCommandActiveCount > 0)
		cRecordingUserCommandActiveCount --;
	char *cmd;
	asprintf(&cmd, "%s %s \"%s\" %d", command, State, *strescape(RecordingFileName, "\"$"), cRecordingUserCommandActiveCount);
...
cFileName::cFileName(const char *FileName, bool Record, bool Blocking)
{
  ...
  record = Record;
  blocking = Blocking;
  if (!FileName) //TB: the strlen below will lead to a segfault if FileName is a NULL-pointer
     return;
