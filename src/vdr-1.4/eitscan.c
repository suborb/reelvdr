...
void cScanList::AddTransponders(cList<cChannel> *Channels)
{
  for (cChannel *ch = Channels->First(); ch; ch = Channels->Next(ch))
      AddTransponder(ch);
  //TB Sort(); // use the order of the channellist
}
                            ...
                            if (Device->ProvidesTransponder(Channel)) {
                               if (!Device->Receiving()) {
                                  ...
                                  if (MaySwitchTransponder || (Device->ProvidesTransponderExclusively(Channel) && now - lastActivity > Setup.EPGScanTimeout * 3600)) {
                                     ...
                                     isyslog("EPG scan: scanning  %-8s tp %5d", *cSource::ToString(Channel->Source()), Channel->Transponder());
                                     Device->SwitchChannel(Channel, false, false);
                                     currentDevice = NULL;
...
#ifdef RBLITE
                      /* TB: sleep at least a bit between channels switches */
                      sleep(1);
#endif
                  }
               }
