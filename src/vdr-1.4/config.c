...
cSetup::cSetup(void)
{
  ...
  FontSmlSize = 14;
  FontFixSize = 16;
  FontSizes = 2; // 0 User defined, 2 Normal, 1 Small fonts
  ...
  ReelboxModeTemp = eModeStandalone; // 0=Standalone, 1=Client, 2=Server, 3=Hotelmode
  ReelboxMode = eModeStandalone; // 0=Standalone, 1=Client, 2=Server, 3=Hotelmode
...
bool cSetup::Load(const char *FileName)
{
  if (cConfig<cSetupLine>::Load(FileName, true)) {
     ...
     for (cSetupLine *l = First(); l; l = Next(l)) {
         ...
         if (error) {
            isyslog("INFO: unknown config parameter: %s%s%s = %s", l->Plugin() ? l->Plugin() : "", l->Plugin() ? "." : "", l->Name(), l->Value());
            result = false;
            }
         }
     return result;
...
static bool configIsPreTrueType = false;

/* TB: compatibility for old lang codes */
const char* ParseOldLanguage(const char *Value)
{
   if (strlen(Value)<=2) {
    int res = atoi(Value);
    configIsPreTrueType = true;
    switch (res) {
      case 0:  return "en_US";
      case 1:  return "de_DE";
      case 2:  return "sl_SI";
      ...
      case 8:  return "es_ES";
      case 9:  return "el_GR";
      case 10: return "sv_SE";
      case 11: return "ro_RO";
      case 12: return "hu_HU";
      case 13: return "ca_ES";
      case 14: return "ru_RU";
      case 15: return "hr_HR";
      case 16: return "et_EE";
      case 17: return "da_DK";
      case 18: return "cs_CZ";
...
{
  if      (!strcasecmp(Name, "OSDLanguage"))         { if(!ParseOldLanguage(Value))
                                                           strn0cpy(OSDLanguage, Value, sizeof(OSDLanguage));
                                                       else
                                                           strn0cpy(OSDLanguage, ParseOldLanguage(Value), sizeof(OSDLanguage));
  ...
  else if ((::Setup.ReelboxModeTemp == eModeStandalone || ::Setup.ReelboxModeTemp == eModeServer)
             && !strcasecmp(Name, "EPGScanTimeout"))
  ...
  */
  ...
  else if (!strcasecmp(Name, "PipSwitchedToTV"))     PipSwitchedToTV    = atoi(Value); 
  ...
  //End by Klaus
     ...
     return false;
  if (configIsPreTrueType) {
     FontOsdSize = 18;
     FontSmlSize = 14;
     FontFixSize = 16;
  }
  if (FontSizes == 3) {  // old "normal"
      FontSizes = 2;
  }
  return true;
}
