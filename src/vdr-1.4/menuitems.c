...
const char *FileNameChars = trNOOP("FileNameChars$ abcdefghijklmnopqrstuvwxyz0123456789-_.,#~\\^$[]|()*+?{}/:%@&");
...
:cMenuEditItem(Name)
{
  ...
  if(Allowed)
    ...
    allowed = (char*)malloc(strlen(tr(FileNameChars))+2);
    if(!strncmp(tr(FileNameChars), "FileNameChars$", 14)) //TB: Hack: strip off the context
        strcpy(allowed, tr(FileNameChars)+14);
    else
        strcpy(allowed, tr(FileNameChars));
    strcat(allowed, "!");
  }
  pos = -1;
  offset = 0;
...
void cMenuEditStrItem::EnterEditMode(void)
{
  if (!valueUtf8) {
     ...
     //original, but does not match our remote control: 
...
void cMenuEditStrItem::Set(void)
{
  if (InEditMode()) {
     ...
     if (!font /* TB: this selects the wrong font || font->Width("W") != 1 */) // all characters have with == 1 in the font used by 'skincurses'
...
eOSState cMenuEditStrItem::ProcessKey(eKeys Key)
{
  ...
  switch (Key) {
    ...
    case kLeft:  if (pos > 0) {
                 ...
                 if (!insert && valueUtf8 && Utf8is(alpha, valueUtf8[pos]))
                    uppercase = Utf8is(upper, valueUtf8[pos]);
                 break;
...
:cMenuEditChanItem(Name, &number, tr("Please select"))
{
  number = 0;
  source = Source;
  transponder = Value;
...
{
  //SetTitle(cString::sprintf("%s - %s", tr("Title$Setup"), Section));
  SetTitle(cString::sprintf("%s", Section));
}
