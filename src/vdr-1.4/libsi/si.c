...
char *String::getText(char *buffer, char *shortVersion, int sizeBuffer, int sizeShortVersion, const char *charset) {
   int len=getLength();
   ...
   decodeText(buffer, shortVersion, sizeBuffer, sizeShortVersion, charset);
   return buffer;
}
...
static const char *getCharacterTable(const unsigned char *&buffer, int &length, bool *isSingleByte = NULL) {
   ...
   //const char *cs = "ISO-8859-9"; // Workaround for broadcaster stupidity: according to
