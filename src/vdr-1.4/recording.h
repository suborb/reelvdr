...
private:
  ...
  bool inHdd_;
  void SetInHdd() { inHdd_ = true;}
  void UnsetInHdd() { inHdd_ = false;}
  bool InHdd() const { return inHdd_; }
public:
  time_t start;
  int priority;
  int lifetime;
  ...
  bool UpdateInfo(bool force = false); // updates cRecordingInfo, called from ScanVideoDir() thread as well
...
private:
  ...
  bool deleted;
  time_t lastUpdate;
  time_t lastModifiedAtLastUpdate;
  int state;
  bool stopScan;
...
protected:
  void Action(void);
  cMutex mutexdotUpdates;
  cStringList dotUpdates; 
  //holds the list of .update file which were found in the video directory
...
public:
  ...
  void StopScan(void) { stopScan = true; }; 
  void Clear(void);
  };
...
char *OldExchangeChars(char *s, bool ToFileSystem); // from vdr 1.4
