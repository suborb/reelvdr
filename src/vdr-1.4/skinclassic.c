...
cSkinClassicDisplayChannel::cSkinClassicDisplayChannel(bool WithInfo)
{
  ...
  PRINTF("font: %p height: %i\n", font, font->Height());
  lineHeight = font->Height();
  message = false;
  osd = cOsdProvider::NewTrueColorOsd(Setup.OSDLeft, Setup.OSDTop + (Setup.ChannelInfoPos ? 0 : Setup.OSDHeight - Lines * lineHeight), 0, 10, 0);
  ...
  tArea Areas[] = { { 0, 0, Setup.OSDWidth - 1, Lines * lineHeight - 1, 32 } };
...
private:
  ...
  int lineHeight;
  cString lastDate;
  void SetScrollbar(int Total = 0, int Offset = 0);
public:
...
int cSkinClassicDisplayMenu::MaxItems(void)
{
  if(lineHeight)
     ...
     esyslog("ERROR: lineHeight == NULL!!!!\n");
     PRINTF("ERROR: lineHeight == NULL!!!!\n");
     return 0;
  }
}
