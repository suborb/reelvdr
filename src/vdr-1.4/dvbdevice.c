...
public:
  ...
  int GetAttribute(const char *attr_name, uint64_t *val);
  int GetAttribute(const char *attr_name, char *val, int maxret);
#endif
  };
...
bool cDvbTuner::SetFrontend(void)
{
  ...
  switch (frontendType) {
    ...
    case FE_QPSK: { // DVB-S
	    ...
	    if (force_diseqc) {
		    // Get a default template for later rotor modification
		    cDiseqc *p=NULL;
		    for (p = Diseqcs.First(); p; p = Diseqcs.Next(p)) {
			    if (p->Slof() > channel.Frequency() && p->Polarization() == toupper(channel.Polarization()) && ( p->Tuner() == cardIndex+1))
				    break;
		    }
		    diseqc = p;
	    }
#endif
            if (diseqc) {
               if (diseqc->Commands() && (!diseqcCommands || force_diseqc || strcmp(diseqcCommands, diseqc->Commands()) != 0)) {
...
#ifndef RB_LITE
				// NetCeiver Rotor
				if (force_diseqc) { // insert new position
				  cmd.msg[3]=cmd3;
				  cmd.msg[4]=cmd4;
				}
#endif
...
#ifdef DEVICE_ATTRIBUTES
int cDvbTuner::GetAttribute(const char *attr_name, uint64_t *val)
{
	int ret=0,v;
	uint64_t rval=0;	

	if (!strcmp(attr_name,"status")) {
		fe_status_t stat=(fe_status_t)0;
		ioctl(fd_frontend, FE_READ_STATUS, &stat);
		rval=stat;
	}
	else if (!strcmp(attr_name,"signal")) {
		ioctl(fd_frontend, FE_READ_SIGNAL_STRENGTH, &v);
		rval=v;
	}
	else if (!strcmp(attr_name,"snr")) {
		ioctl(fd_frontend, FE_READ_SNR, &v);
		rval=v;		
	}
	else if (!strcmp(attr_name,"ber")) {
		ioctl(fd_frontend, FE_READ_BER, &v);
		rval=v;		
	}
	else if (!strcmp(attr_name,"unc")) {
		ioctl(fd_frontend, FE_READ_UNCORRECTED_BLOCKS, &v);
		rval=v;		
	}
...
int cDvbTuner::GetAttribute(const char *attr_name, char *val, int maxret)
{
	int ret=0;
	if (!strcmp(attr_name,"name")) {
		struct dvb_frontend_info info;
		ioctl(fd_frontend, FE_GET_INFO, &info);
		if (val && maxret>0) {
			strncpy(val,info.name,maxret);
			val[maxret-1]=0;
		}
	}
	else
  ...
  if (fd_frontend >= 0) {
     ...
     if (ioctl(fd_frontend, FE_GET_INFO, &feinfo) >= 0) {
        ...
        if (strstr(feinfo.name,"DVB-S2"))
          frontendType = (fe_type_t) FE_DVBS2;
        else
...
bool cDvbDevice::HasInput(void) const
{
  return true;
}
...
{
#ifdef DVBAVI_V5
   return (frontendInfo.caps & FE_CAN_2ND_GEN_MODULATION);
#else
...
{
  return ProvidesSource(Channel->Source()) && ((Channel->Modulation()==QPSK_S2 || Channel->Modulation()==PSK_8) ? (frontendType==FE_DVBS2) : true) && (!cSource::IsSat(Channel->Source()) || !Setup.DiSEqC || Diseqcs.Get(Channel->Source(), Channel->Frequency(), Channel->Polarization(), CardIndex()+1));  
}
...
bool cDvbDevice::ProvidesChannel(const cChannel *Channel, int Priority, bool *NeedsDetachReceivers) const
{
  ...
  if (ProvidesSource(Channel->Source()) && ((Channel->Modulation()==QPSK_S2 || Channel->Modulation()==PSK_8) ? (frontendType==FE_DVBS2) : true) && ProvidesCa(Channel) ) { // TB: hack - only use DVBS2-tuners for S2-channels
     result = hasPriority;
...
bool cDvbDevice::SetChannelDevice(const cChannel *Channel, bool LiveView)
{
  ...
  bool StartTransferMode = IsPrimaryDevice() && !DoTune && !Setup.LiveBuffer
                           && (LiveView && HasPid(vpid ? vpid : apid) 
                           && (!pidHandlesVideo || (!pidHandlesAudio && (dpid ? pidHandles[ptAudio].pid != dpid : true)))// the PID is already set as DMX_PES_OTHER
  ...
  if (TurnOnLivePIDs) {
     ...
     cControl::Launch(new cTransferControl(this, vpid, Channel->Ppid(), Channel->Apids(), Channel->Dpids(), Channel->Spids()));
...
/* Attribute classes for dvbdevice
  ...
  fe : frontend attributes (-> get from tuner)
      ...
      .status,.snr,... (int)
*/
int cDvbDevice::GetAttribute(const char *attr_name, uint64_t *val)
{
	int ret=0;
	uint64_t rval=0;	
	
	if (!strncmp(attr_name,"fe.",3)) {		
		if (dvbTuner)
			return dvbTuner->GetAttribute(attr_name+3,val);
		else
			return 0;
	}
...
int cDvbDevice::GetAttribute(const char *attr_name, char *val, int maxret)
{
	
	if (!strncmp(attr_name,"fe.",3)) {		
		if (dvbTuner)
			return dvbTuner->GetAttribute(attr_name+3,val, maxret);
		else
			return 0;
	}
	else if (!strncmp(attr_name,"main.",5)) {
