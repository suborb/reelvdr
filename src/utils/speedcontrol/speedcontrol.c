 ...
 *
 *
 * Copyright (c) 2004   Thomas Fritzsche <t...@noto.de>
 *
...
void dump_sense(unsigned char *cdb, struct request_sense *sense)
{
  ...
  printf("speedcontrol: Command failed: ");
...
int main(int argc, char *argv[])
{
  ...
  if (fd < 0) {
    printf("speedcontrol: Can't open device %s\n", device);
    return -1;
  }
...
/* write time 1 sec. */
  ...
  if (ioctl(fd, CDROM_SEND_PACKET, &cgc) != 0)
    if (ioctl(fd, CDROM_SELECT_SPEED, speed) != 0) {
      ...
      printf("speedcontrol: ERROR.\n");
      return -1;
    }
  printf("speedcontrol: speed set to '%d'...\n", speed);
  return 0;
} 
