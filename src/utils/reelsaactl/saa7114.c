 ...
 *
 * Modification for userspace library for Reelbox PVR
 *   by Georg Acher (acher at baycom dot de)
 *
...
#define SAA_7114_PAL_HEIGHT             (312)   // GA, was: 300
...
/* ----------------------------------------------------------------------- */
// KERNEL ENVIRONMENT EMULATION

#define kmalloc(a,b)  malloc(a)
#define kfree(a) free(a)
#define i2c_get_clientdata(x) (x->data)
#define i2c_set_clientdata(a,b) a->data=b
#define i2c_check_functionality(a,b) 1

#define KERN_ERR
#define KERN_INFO
#define KERN_DEBUG
#define I2C_CLIENT_END 0
#define I2C_CLIENT_ALLOW_USE 0
...
int i2c_attach_client(struct i2c_client* a)
{
	a->adapter->client=a;
	return 0;
}

// Filedescriptor for dev/i2c
static int fd;
...
{
        int a;
	struct saa7114_handle *handle=(struct saa7114_handle *)malloc(sizeof(struct saa7114_handle));
	int i;

	if (getuid()==0) {
		iopl(3);
		a=inl(0x9030); // PMR
		a&=~0x80000; // make ab2 GPIO
		outl(a,0x9030);
	}
	else {
		fprintf(stderr,"libsaa7114: Can't enable Geode I2C-pins as non-root\n");
	}
        fd=open(dev,O_RDWR);
        if (fd<0) 
        {
                fprintf(stderr,"libsaa7114: Can't open %s\n",dev);
		free(handle);
		return 0;
        }
	handle->fd=fd;
	saa7114_detect_client(&handle->adap, I2C_SAA7114/2, dont_reset);
        return handle;
}
/* ----------------------------------------------------------------------- */
...
{
	saa7114_write(handle->adap.client,reg,val);
}
/* ----------------------------------------------------------------------- */
int SAA7114_read (struct saa7114_handle* handle, u8 reg) 
{
	return saa7114_read(handle->adap.client,reg);
}
/* ----------------------------------------------------------------------- */
...
{
	int res;
	res= saa7114_command(handle->adap.client,cmd,arg);
	return res;
}
