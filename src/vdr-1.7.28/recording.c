...
#endif
#define INFOFILESUFFIX    "/info" // in reelvdr 1.7 it becomes "/info.txt"
     ...
     while ((s = ReadLine.Read(f)) != NULL) {
           ...
           switch (*s) {
             ...
             default: if (!ownEvent->Parse(s)) {
                         esyslog("ERROR: Recording-info data problem in line %d", line);
                         return false;
                         }
                      break;
             }
...
#ifdef REELVDR
     cString InfoFileName = cString::sprintf("%s%s", fileName, isPesRecording ? INFOFILESUFFIX ".vdr" : INFOFILESUFFIX ".txt");
#else
...
{
#if REELVDR
 /// run this thread slow;
 //  we do not know how many files are in the 'recordings directory'
 //  params copied from cRemoveDeletedRecordingsThread::Action(void)
  SetPriority(19);
  SetIOPriority(7);
#endif
  Refresh();
...
void cRecordingUserCommand::InvokeCommand(const char *State, const char *RecordingFileName, const char *SourceFileName, char *chanName)
#else
...
#endif /* DVLRECSCRIPTADDON */
{
  if (command) {
    ...
    cString cmd;
    if (SourceFileName)
       cmd = cString::sprintf("%s %s \"%s\" \"%s\" \"%s\"", command, State, *strescape(RecordingFileName, "\\\"$"), *strescape(SourceFileName, "\\\"$"));
    else
       cmd = cString::sprintf("%s %s \"%s\" \"%s\"", command, State, *strescape(RecordingFileName, "\\\"$"));
    }
     else {
    cString cmd;
    if (SourceFileName)
        ...
        if (stat(fileName, &buf) == 0) {
           ...
           if (!Record && last >= 0) {
              ...
              if (index) {
                 ...
                 if (f >= 0) {
#if 0 //def REELVDR //RC: TODO: re-intruduce this patch, avoids crash! svn r16772
              if (!Record && !buf.st_size) { // Accessing a file that just started recording (PauseLiveVideo)
                 time_t tmax = time(NULL) + MAXWAITFORINDEXFILE;
                 do { // Wait some time for index to get filled
                    if((stat(fileName, &buf) == 0) && buf.st_size)
                       break;
                    cCondWait::SleepMs(INDEXFILECHECKINTERVAL);
                 } while (time(NULL) < tmax);
              } // if
#endif
