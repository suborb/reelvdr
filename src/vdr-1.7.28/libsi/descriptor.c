...
char *ExtendedEventDescriptors::getText(char *buffer, int size, const char* charset, const char *separation1, const char *separation2) {
#else
char *ExtendedEventDescriptors::getText(char *buffer, int size, const char *separation1, const char *separation2) {
   ...
   for (int i=0;i<length;i++) {
      ...
      if (!d)
         continue;
#ifdef USE_PROVIDERCHARSET
      d->text.getText(buffer+index, size, charset);
#else
...
#ifdef USE_PROVIDERCHARSET
         item.itemDescription.getText(buffer+index, size, charset);
#else
...
#ifdef USE_PROVIDERCHARSET
         item.item.getText(buffer+index, size, charset);
#else
...
#ifdef USE_PROVIDERCHARSET
char *ExtendedEventDescriptors::getTextPlain(const char* charset) {
    int size = getMaximumTextPlainLength();
    char *text=new char[size];
    return getTextPlain(text, size, charset);
 }
#else
...
#ifdef USE_PROVIDERCHARSET
char *ExtendedEventDescriptors::getTextPlain(char *buffer, int size, const char* charset) {
#else
...
#ifdef USE_PROVIDERCHARSET
char *ExtendedEventDescriptors::getTextItemized(char *buffer, int size, const char* charset, const char *separation1, const char *separation2) {
#else
...
#ifdef USE_PROVIDERCHARSET
bool ExtendedEventDescriptors::getTextItemized(Loop::Iterator &it, bool &valid, char *itemDescription, char *itemText, int sizeItemDescription, int sizeItemText, const char *charset) {
#else
...
#ifdef USE_PROVIDERCHARSET
         item.item.getText(itemDescription, sizeItemDescription, charset);
         item.itemDescription.getText(itemText, sizeItemText, charset);
#else
