   ...
   T* getNextAsPointer(Iterator &it)
      {
         ...
         ret->CheckParse();
         if (!checkSize(ret->getLength())) {
#if REELVDR // reported to kls; avoid memory leak
             delete ret;
#endif
            return 0;
         }
...
public:
   ...
   //returns text. Data is allocated with new and must be delete'd by the user.
    ...
    char *getText(const char* charset=NULL);
#else
   char *getText();
#endif
...
#ifdef USE_PROVIDERCHARSET
   char *getText(char *buffer, int size, const char* charset=NULL);
#else
...
#ifdef USE_PROVIDERCHARSET
   char *getText(char *buffer, char *shortVersion, int sizeBuffer, int sizeShortVersion, const char* charset=NULL);
#else
...
protected:
   virtual void Parse() {}
#ifdef USE_PROVIDERCHARSET
   void decodeText(char *buffer, int size, const char* charset=NULL);
   void decodeText(char *buffer, char *shortVersion, int sizeBuffer, int sizeShortVersion, const char* charset=NULL);
#else
