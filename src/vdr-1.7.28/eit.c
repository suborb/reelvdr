...
#ifdef USE_PROVIDERCHARSET
const char *charset = ProviderCharsets.GetCharset(channel->Provider());
#endif
  ...
  if (!allowedEPG(kanalID))
     OnlyRunningStatus = true; // Do not update EPG - but collect status information
#endif /* NOEPG */
  ...
  for (SI::Loop::Iterator it; eventLoop.getNext(SiEitEvent, it); ) {
...
#ifdef USE_PROVIDERCHARSET
                    Components->SetComponent(Components->NumComponents(), Stream, Type, I18nNormalizeLanguageCode(cd->languageCode), cd->description.getText(buffer, sizeof(buffer), charset));
#else
      ...
      if (!rEvent) {
         if (ShortEventDescriptor) {
            char buffer[Utf8BufSize(256)];
#ifdef USE_PROVIDERCHARSET
            EpgHandlers.SetTitle(pEvent, ShortEventDescriptor->name.getText(buffer, sizeof(buffer), charset));
            EpgHandlers.SetShortText(pEvent, ShortEventDescriptor->text.getText(buffer, sizeof(buffer), charset));
#else
...
#ifdef USE_PROVIDERCHARSET
            EpgHandlers.SetDescription(pEvent, ExtendedEventDescriptors->getText(buffer, sizeof(buffer), charset, ": "));
#else
