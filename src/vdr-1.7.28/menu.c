...
#ifdef USE_WAREAGLEICON
           buffer = cString::sprintf("%d\t%.*s\t%s\t%s\t%s%s%s\t%s", channel->Number(), Utf8SymChars(csn, 999), csn, *event->GetTimeString(), ProgressBar[progress], t, v, r, event->Title());
#else
           buffer = cString::sprintf("%d\t%.*s\t%s\t%s\t%c%c%c\t%s", channel->Number(), Utf8SymChars(csn, 999), csn, *event->GetTimeString(), ProgressBar[progress], t, v, r, event->Title());
#endif /* WAREAGLEICON */
           }
        else
#ifdef USE_WAREAGLEICON
...
#ifdef USE_JUMPPLAY
:cDvbPlayerControl(fileName, PauseLive), marks(fileName)
#else
