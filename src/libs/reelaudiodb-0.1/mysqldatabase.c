...
// squash a vector of strings to a comma seperated string
...
bool cMysqlDatabase::Insert(const std::string& table, const cFields &field, const cRow &row)
{
    ...
    // convert vector of strings into a comma seperated string (str1, str2, str3 ...)
...
bool cMysqlDatabase::ExecuteSQLQuery(const std::string& query, cResults *results)
{
    ...
    if (res)
    {
        ...
        results->fields.reserve(num_fields);
        for (i=0; i < num_fields; ++i)
        {
            results->fields.push_back(fields[i].name?fields[i].name:"");
        }

        results->results.reserve(mysql_num_rows(res));
        cRow r;
        r.reserve(num_fields); // alloca memory once 
        // get the resulting rows
        while ((row = mysql_fetch_row(res)))
        {
            r.clear(); // clear previous 'row' and use the same memory location
            for (i=0; i < num_fields; ++i)
            {
...
cResults cMysqlDatabase::Query(const std::string& table , const cQuery &q)
{
    std::string sqlQuery;
    cResults result;
