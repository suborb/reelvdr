    ...
    */
    ...
    for (unsigned int i = 0; i < str.size(); ++i)
    {
        if (str[i] == '\'') // single quote
            result.push_back('\\'); // push a backslash before single quotes
...
cFileInfo cSearchDatabase::FileInfo(const std::string &path)
{
    ...
    cResults result;
    bool res = reader->Query(result, "metadata", q);
    std::deque<cRow>::const_iterator it = result.results.begin();
...
std::deque<cFileInfo> cSearchDatabase::Files(const std::string& genre, const std::string& artist, const std::string& album, const std::string& title)
{

    if (!reader)
        return std::deque<cFileInfo>();
    ...
    // copy the titles into one deque from 'result'
    std::deque<cFileInfo> g;
    std::deque<cRow>::const_iterator it = result.results.begin();
    for ( ; it != result.results.end(); ++it)
    {
        if (it->size() != 7 /*number of 'AddField()' */)
...
std::deque<std::string> cSearchDatabase::Title(const std::string& genre, const std::string& artist, const std::string& album, const std::string& title)
{
...
std::deque<std::string> cSearchDatabase::Tracks(const std::string& genre, const std::string& artist, const std::string& album, const std::string& filename)
{
    ...
    // copy the paths into one deque from 'result'
    std::deque<std::string> g;
    std::deque<cRow>::const_iterator it = result.results.begin();
    for ( ; it != result.results.end(); ++it)
    {
...
std::deque<std::string> cSearchDatabase::Albums(const std::string& genre, const std::string& artist, const std::string& album)
{
    if (!reader)
        return std::deque<std::string>();
    ...
    // copy the genres into one deque from 'result'
    std::deque<std::string> g;
    std::deque<cRow>::const_iterator it = result.results.begin();
    for ( ; it != result.results.end(); ++it)
    {
...
std::deque<std::string> cSearchDatabase::Artists(const std::string& genre, const std::string& artist)
{
    if (!reader)
        return std::deque<std::string>();
...
std::deque<std::string> cSearchDatabase::Genres(const std::string &genre)
{
    if (!reader)
        return std::deque<std::string>();
