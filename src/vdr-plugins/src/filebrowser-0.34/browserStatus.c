...
void cFileBrowserStatus::Shutdown(std::string dir, std::string file, /*std::string playlist,*/ std::string playListEntry)
{
    ...
    if(!(mode_ == restricted || mode_ == browseplaylist || stillPictureActive_ || (ifTypes_.size() && ifTypes_[ifTypes_.size() - 1] == if_xine)))
    { 
        //delete current instance at next startup 
        valid_ = false;
    }

    if(normalExit_ && !calledFromMenu_)
    { 
...
{
    filter_ = cl_unknown;
    if(startDir_ == PICTURES_DIR && caller_ == mediaplayer)
    {
