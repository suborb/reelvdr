...
eOSState cFileTypeBase::UpdateFile(cPlayListItem &item, cMenuFileBrowserBase *menu, bool currentChanched) const
{
    printf("-------eOSState cFileTypeBase::UpdateFile-----------\n");
    menu->ShowThumbnail(item, true, currentChanched);
    menu->ShowID3TagInfo(item, true, currentChanched);
    return osContinue;
}

eOSState cFileTypeBase::OpenFile(cPlayListItem &item, cMenuFileBrowserBase *menu, bool asPlaylist) const
{
    std::string message = tr("Can't open file");
