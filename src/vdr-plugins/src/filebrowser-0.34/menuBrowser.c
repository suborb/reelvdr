...
: cMenuFileBrowserBase(title), currentdir_(newstatus->GetStartDir()), cache_(cache), convert_(convert), thumbConvert_(thumbConvert), playlist_(newstatus->GetPlayList())
{
    ...
    {
        //printf("#####cMenuFileBrowser: ClosePlayList\n");
...
{ 
    //printf("-------------cMenuFileBrowser::Shutdown-----------------\n");
...
eOSState cMenuFileBrowser::ShowPlayListWithXine()
{
     //printf("---------cMenuFileBrowser::ShowPlayListWithXine--------\n");
...
eOSState cMenuFileBrowser::ShowFileWithXine(const cPlayListItem &item, bool asPlaylistItem)
{
...
eOSState cMenuFileBrowser::ShowThumbnail(cPlayListItem item, bool hide, bool currentChanched)
{
...
{   
    ...
    playlist_->Insert(markedEntries_);
...
bool cMenuFileBrowser::HasUnfinishedTasks()
{
...
void cMenuFileBrowser::SavePlayList(std::string &path)
{
    //printf("----------cMenuFileBrowser::SavePlaylist--------\n");
    playlist_->SaveToFile();
    path = playlist_->GetName();
    ClearPlayList();
}

void cMenuFileBrowser::CopyPlayListToUsb()
{
...
void cMenuFileBrowser::CopyPlayListToDvd()
{
    playlist_->SaveToFile(); //?????????????
...
void cMenuFileBrowser::FlagItems()
{
    cMenuFileItem *fileitem;
    for (int i = 0; i < Count(); i++)
    {
        ...
        if(fileitem)
        {
            if(status->GetMode() == browseplaylist && playlist_->IsInList(fileitem->GetPlayListItem()))
            {
                fileitem->Flag();
            }
            else
...
bool cMenuFileBrowser::IsForbiddenDirEntry(std::string entry) const
{
    ...
    if(status->GetMode() == expert || status->GetMode() == browseplaylist)
    {
