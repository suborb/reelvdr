...
void cConflictCheck::Check()
{
    ...
    if (handler) {
       for (int Slot = 0; Slot < handler->NumSlots(); Slot++) {
             ...
             if (cas) { 
                if ((cas && cas->GetState() < 2 && cas->GetState() >= 0)){
		   PRINTF("state: %i\n", cas->GetState());
                   allCAMsReady = false;
                } else
		   count++;
       ...
       if(allCAMsReady==false || count != handler->NumCams()){
          isyslog("cConflictCheck::Check() is waiting until all CAMs are ready\n");
          PRINTF("count: %i numcams: %i ready: %i\n", count, handler->NumCams(), allCAMsReady);
