...
:cOsdMenu(tr("Search templates"), 3, 10, 6, 9, 6)
{
    search = Search;
    blacklist = Blacklist;
    newSearch = New;
    ...
    SetHelp(trVDR("Button$Edit"), trVDR("Button$Delete"), trVDR("Button$New"), NULL /*tr("Button$Default")*/);
    } else {
	Add(new cOsdItem(tr("No templates present."),osUnknown, false));
	AddFloatingText(tr("Use the yellow button to create a new template."), MAXOSDTEXTWIDTH);
	SetHelp(NULL, NULL, trVDR("Button$New"), NULL);
    }	
    Sort();
    Display();
...
eOSState cMenuEPGSearchTemplate::ProcessKey(eKeys Key)
{
  ...
  if (state == osUnknown) {
	  ...
	  case kRed:
		  ...
		  state = osContinue;
	      break;
	  case kYellow: state = New(); break;
	  case kGreen: state = Delete(); break;
