...
cString IndentMenuItem(const char*);
#if 0
...
inline char *strreplaceall(char* source, const char ch, const char replacement)
{
   for (char *sptr = source; *sptr; sptr++) {
      if (*sptr == ch)
         *sptr = replacement;
   }
   return (source);
...
inline void sleepMSec(long ms)
{
   cCondWait::SleepMs(ms);
}

inline void sleepSec(long s)
{
   sleepMSec(s * 1000);
}
