...
#ifdef REELVDR
			} else if(EPGSearchConfig.showPipOnAnnounce) {
				int lData = channel->Number();
				if(cPluginManager::CallAllServices("is pip channel", (void *)&lData))
					cPluginManager::CallAllServices("show pip channel", (void *)&lData);
#endif
			}
