...
void cTemplFile::PrepareDefaultTemplates()
{
    ...
    {
    // Hint: SetCols(...) for EPG - Now
#if APIVERSNUM < 10700
        sprintf(menutemplate, "%s%%chlng%%:12|%%time%%:6|%s%s$status$:4|%%title%% ~ %%subtitle%%:30",
#else
        sprintf(menutemplate, "%s%%chlng%%:14|%%time%%:6|%s%s$status$:5|%%title%% ~ %%subtitle%%:30",
#endif
		EPGSearchConfig.showChannelNr?channelnr:"",
		EPGSearchConfig.showProgress==0?"":(EPGSearchConfig.showProgress==1?"%progrT2S%:5|":"%progr%:5|"),
		text2skin?" ":"");
	WhatsOnNow->PrepareTemplate(menutemplate);
    }

    // What's on next and user defined starting time
#if APIVERSNUM < 10700
        sprintf(menutemplate, "%s%%chlng%%:12|%%time%%:7|$status$:4|%%title%% ~ %%subtitle%%:30",
#else
        sprintf(menutemplate, "%s%%chlng%%:14|%%time%%:7|$status$:5|%%title%% ~ %%subtitle%%:30",
#endif
	    EPGSearchConfig.showChannelNr?channelnr:"");
    ...
    if (Schedule && Schedule->MenuTemplate() == 0)
    {
        ...
        if (EPGSearchConfig.showDaySeparators)
            strcpy(menutemplate, "%time%:6|$status$:5|%title% ~ %subtitle%:30");
        else
        strcpy(menutemplate, "%time_w% %time_d%:8|%time%:6|$status$:5|%title% ~ %subtitle%:30");
#endif
    ...
    {
#if APIVERSNUM < 10700
    //TB sprintf(menutemplate, "%s%%chsh%%:12|%%time%%:6|%%timespan%%:7|$status$:3|%%title%% ~ %%subtitle%%:30",
        sprintf(menutemplate, "%s%%chlng%%:12|%%time%%:6|%%date%%:7|$status$:3|%%title%% ~ %%subtitle%%:30",
#else
        sprintf(menutemplate, "%%title%% ~ %%subtitle%%:25|%%date%%:6|%%time%%:6|$status$:5|%s%%chlng%%:20",
#endif
		EPGSearchConfig.showChannelNr ? channelnr : "");
    ...
    {
	sprintf(menutemplate, "%s%%chlng%%:12|%%time%%:6|%%timespan%%:7|%%status%%:3|%%title%% ~ %%subtitle%%:30",
		EPGSearchConfig.showChannelNr?channelnr:"");
