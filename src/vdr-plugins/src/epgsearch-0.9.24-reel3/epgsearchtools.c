...
cString IndentMenuItem(const char* szString)
{
   unsigned int len = strlen(szString);
   char* szIndented = (char*)malloc(len+3);
   szIndented[0] = szIndented[1] = ' ';
   strncpy(szIndented+2, szString, len);
   szIndented[len+2] = '\0';
   cString szIndentedStr(szIndented, true);
