...
void cSearchTimerThread::Action(void) 
{
   ...
   {
#ifdef USEMYSQL
       if(Setup.ReelboxModeTemp!=eModeClient) // Don't run this if the Reelbox is a Client
       {
#endif
      ...
      {
#ifdef USEMYSQL
          if(Setup.ReelboxModeTemp==eModeServer) // Server only
              SearchExts.SyncData();
#endif
          sleepSec(1);
      }
...
char* cSearchTimerThread::SummaryExtended(cSearchExt* searchExt, cTimer* Timer, const cEvent* pEvent)
{
   ...
   {
      tmpaux = strdup(aux);
      char* begin = (char *) strstr(aux, "<epgsearch>");
      char* end   = (char *) strstr(aux, "</epgsearch>");
      if (begin && end)
      {
         if (begin == aux) 
             strcpy(tmpaux, ""); 
         else 
             strn0cpy(tmpaux, aux, begin-aux+1);
