...
class cMenuDirItem : public cOsdItem 
{
 ...
 public:
    cMenuDirItem(const char* text) : cOsdItem(text) { directory = strdup(text);
                                                      SetText(cString::sprintf("%c %s", char(130), directory));}
    ~cMenuDirItem(){ if (directory) free(directory);}
    char* Directory() { return directory; }
...
eOSState cMenuDirSelect::ProcessKey(eKeys Key)
{
  ...
  if (state == osUnknown) {
	 ...
	 case kBlue:
	 case kOk:  {
         cMenuDirItem *item = dynamic_cast<cMenuDirItem*> (Get(Current()));
	     if (item && item->Directory())
             strn0cpy(Directory, item->Directory(), MaxFileName);
     }
	     return osBack;
       default:   break;
       }
