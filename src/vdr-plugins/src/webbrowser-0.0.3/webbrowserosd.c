...
cWebbrowserOsd::cWebbrowserOsd():cOsdMenu("")
{
    ...
    case MODE_CHROME:
        {
            switchToHDifAutoFormat();
            buf << STARTFBX << " " << resString << " chrome " << WebbrowserSetup.FFstartURL;
            //std::cout << "executing: " << buf.str() << std::endl;
            SystemExec(buf.str().c_str());
            SetFbParams();
            break;
...
cWebbrowserOsd::~cWebbrowserOsd()
{
    if (HasDevice("80860709")) // RB ICE
    {
        ...
        //TODO: on RB ICE do not switch back to live tv when menu is closed
        SystemExec("/usr/sbin/zorder B A C D");
    }
    else
    {
    std::string buf;
