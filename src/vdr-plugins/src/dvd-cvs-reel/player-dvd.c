...
cDvdPlayer::cDvdPlayer(void): cThread("dvd-plugin"), a52dec(*this) {
    ...
    // blocks vdr threads during DVD replay
    cRecordings::DVDPlayerActive = true;
    oldEPGScanTimeout = Setup.EPGScanTimeout;
    Setup.EPGScanTimeout = 0;
...
cDvdPlayer::~cDvdPlayer()
{
    ...
    ::Setup.EPGScanTimeout = oldEPGScanTimeout;
    cRecordings::DVDPlayerActive = false;
}
...
void cDvdPlayer::Action(void) {
    ...
    DVDSetup.MenuLanguage = Setup.OSDLanguage;
    DVDSetup.AudioLanguage = Setup.AudioLanguages[0];
    DVDSetup.SpuLanguage = Setup.OSDLanguage;
    ...
    if(HasBitStreamOut) {
	    ...
	    if(slBitStreamOutActive!=NULL)
		    ...
		    */
	BitStreamOutActive = Setup.UseDolbyDigital ? true : false;
    }
...
//    IframeCnt = -1; // mark that we have to reset the device, before 1st PlayVideo ..
    ...
    /*bool*/ next_cell_change = false; //Mod by Klaus
    
    // and seek the the exact time stored here
    /*int64_t*/ resSecond = 0; //Mod by Klaus
...
// GA: Speedup
int cDvdPlayer::playPacketHelper(uchar* &data, int &datalen, uint8_t &currentFrameType, bool do_copy, bool trickMode,
...
//		    stcPTS=(uint64_t)rawSTC;
    ...
    switch (cPStream::packetType(sector)) {
		...
		int n;
	        while (datalen > 6) {
			n=FindPacketHeader(data,0,datalen);
			if (n>=0) {
				data+=n;
				datalen-=n;
				if (playPacketHelper(data, datalen, currentFrameType, do_copy, trickMode,
						     havePictureHeader, haveSequenceHeader, haveSliceBeforePicture))
					break;
				else {
					data+=4;
					datalen-=4;
...
//Start by Klaus
void cDvdPlayer::GotoFilm()
{ 
      if(IsInMenuDomain())
      { 
          StillSkip();
          GotoTitle(lasttitle, lastpart); 
...
//Start by Klaus
void  cDvdPlayer::SetTitlePosition()
{
    dvdnav_current_title_info(nav, &lasttitle, &lastpart); 
    int64_t temp;
    GetPositionInSec(lastsec, temp);
}
//End by Klaus
