...
cEPGSearchConfig::cEPGSearchConfig(void)
{
  ...
  checkTimerConflictsAfterUpdate = 1;
  checkMinPriority = 0;
  checkMinDuration = 0;
  checkMaxDays = 99;
  ignorePayTV     = 0;
  ...
  checkTimerConflAfterTimerProg = 1;
  ...
  noAnnounceWhileReplay = 1;
  TimerProgRepeat = 0;
  maxChannelMenuNow = 0;
  noConflMsgWhileReplay = 0;

  ShowModes[showUserMode1].SetDescription("20:15");
  ShowModes[showUserMode1].SetTime(2015);
  ShowModes[showUserMode1].SetUsage(1);

  ShowModes[showUserMode2].SetDescription("22:00");
  ShowModes[showUserMode2].SetTime(2200);
  ShowModes[showUserMode2].SetUsage(1);
}
