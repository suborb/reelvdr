...
void cConflictCheckThread::Action(void)
{
    ...
    if(!cPluginManager::GetPlugin("mcli")) {
        while(allCAMsReady==false){
            allCAMsReady = true;
            for (i=0; i<cDevice::NumDevices(); i++) {
                cCiHandler *ciHandler = cDevice::GetDevice(i)->CiHandler();
                if (ciHandler) {
                    allCAMsReady &= ciHandler->Ready();
                }
            }
            if(allCAMsReady==false){
                isyslog("cConflictCheckThread sleeping until CAMs are ready\n");
...
#else
   int secs = 10;
   printf("epgsearch [%s:%d] ======== cDevice::NumDevices() %d =======\n",
           __FILE__, __LINE__, cDevice::NumDevices());
   printf("epgsearch [%s:%d] sleeping for %d secs so that mcli devices are available to vdr\n", 
          __FILE__,__LINE__, secs);

   cCondWait::SleepMs(secs*1000);

   printf("epgsearch [%s:%d] after sleeping %d secs ======== cDevice::NumDevices() %d =======\n",
           __FILE__, __LINE__, secs, cDevice::NumDevices());
#endif
	...
	if (now >= nextUpdate || m_forceUpdate)
	{
		...
		if (doMessage && SendMsg(msgfmt, true, 8) == kOk)
		{
