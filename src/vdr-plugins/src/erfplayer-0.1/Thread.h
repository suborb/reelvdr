...
namespace Reel
{
    template < typename T > class Thread        /* final */
    {
      public:
        Thread();
        ~Thread() NO_THROW;
  ...
  template < typename T > inline Thread < T >::Thread():obj_(0)
    {
    }

    template < typename T > inline Thread < T >::~Thread()
    {
        REEL_ASSERT(!obj_);
    }

    template < typename T > inline void Thread < T >::Join()
    {
        if (!obj_) return;
    ...
    template < typename T > void Thread < T >::Start(T & obj, MemFunc memFunc)
    {
        obj_ = &obj;
        memFunc_ = memFunc;
    ...
    template < typename T > void *Thread < T >::StartProc(void *arg)
    {
