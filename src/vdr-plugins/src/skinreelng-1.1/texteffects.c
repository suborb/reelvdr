...
cReelTextEffects ReelTextEffects;
//#else
//cReelTextEffects ReelTextEffects("ReelNG effects");
//#endif

//#ifdef DISABLE_ANIMATED_TEXT
cReelTextEffects::cReelTextEffects(void) : osd(NULL)
//#else
//cReelTextEffects::cReelTextEffects(const char *Description) : cThread(Description), osd(NULL), condSleep(), mutexSleep(), mutexRunning()
//#endif
{
//  SetPriority(19);
...
cReelTextEffects::~cReelTextEffects(void)
{
//#ifndef DISABLE_ANIMATED_TEXT
////TODO?  Stop();
//#endif
...
//#ifndef DISABLE_ANIMATED_TEXT
//void cReelTextEffects::Action(void)
//{
//  mutexRunning.Lock();
//  mutexSleep.Lock();
//
//  debug("cReelTextEffects::Action() %lu", pthread_self());
//
//  while (ReelConfig.useTextEffects && osd) {
...
//
//      if (e->nNextUpdate == 0) {
//        e->nNextUpdate = nNow + (e->nAction == 0 ? ReelConfig.scrollPause : ReelConfig.blinkPause);
...
//      else
//        condSleep.TimedWait(mutexSleep, ReelConfig.scrollPause); //TODO
////      printf("SLEEP2: %d, %lu\n", nSleepMs, pthread_self());
//    }
//  }
//
...
//}
//
//void cReelTextEffects::DoEffect(tEffect *e, uint64_t nNow)
//{
...
//}
//
//void cReelTextEffects::DoScroll(tEffect *e, uint64_t nNow, bool fDrawItem)
//{
////  debug("cReelTextEffects::DoScroll()");
//  const cFont *Font = ReelConfig.GetFont(e->FontId, e->Font);
...
//
//    if (nNow)
//      e->nNextUpdate = nNow + ReelConfig.scrollPause;
//    return;
//  }
//
//  if (nNow) {
//    int nDelay = ReelConfig.scrollDelay;
//    if (fDrawItem) {
//      switch (e->nDirection) {
//        case 0: // Scroll from left to right
//          if (Font->Width(e->strText.c_str() + e->nOffset) <= e->Width) {
//            if (ReelConfig.scrollMode)
//              e->nDirection = 2;
//            else
//              e->nDirection = 1;
//            nDelay = ReelConfig.scrollPause;
...
//}
//
//void cReelTextEffects::DoBlink(tEffect *e, uint64_t nNow, bool fDrawItem)
//{
////  debug("cReelTextEffects::DoBlink()");
//  const cFont *Font = ReelConfig.GetFont(e->FontId, e->Font);
//  if (fDrawItem) {
//    if (nNow) {
//      e->nDirection = (e->nDirection == 0 ? 1 : 0);
//      e->nNextUpdate = nNow + ReelConfig.blinkPause;
//    }
//    if (e->nDirection == 1)
...
//}
//
//bool cReelTextEffects::Start(cOsd *o)
//{
//  osd = o;
//
//  if (!ReelConfig.useTextEffects)
//    return false;
//
//  debug("cReelTextEffects::Start(%p) %lu", osd, pthread_self());
//
//  if (osd == NULL)
//    return false;
//
//  if (Running()) {
//    error("cReelTextEffects::Start - already running");
...
//}
//
//void cReelTextEffects::Stop(void)
//{
//  //Must be TE_LOCKed by caller (calls TE_UNLOCK)
//
//  debug("cReelTextEffects::Stop()");
//  osd = NULL;
//  Clear();
//  TE_UNLOCK;
...
//}
//
//void cReelTextEffects::Clear(void)
//{
//  debug("cReelTextEffects::Clear()");
//
...
//}
//
//void cReelTextEffects::PauseEffects(int y)
//{
////  debug("cReelTextEffects::PauseEffects(%d)", y);
//
//  //Must be TE_LOCKed by caller
//
//  yMessageTop = y;
//}
//
//void cReelTextEffects::ResetText(int i, tColor ColorFg, tColor ColorBg, bool fDraw)
//{
//  debug("cReelTextEffects::ResetText(%d)", i);
//
//  //Must be TE_LOCKed by caller
//
...
//}
//
//void cReelTextEffects::UpdateTextWidth(int i, int Width)
//{
//  debug("cReelTextEffects::UpdateTextWidth(%d)", i);
//
//  //Must be TE_LOCKed by caller
//
//  if (i < 0 || i >= (int)vecEffects.size())
...
//}
//
//int cReelTextEffects::DrawAnimatedTitle(int o_id, int action, const char *s, int Width, cSkinReelThreadedOsd *skin)
//{
//  //Must be TE_LOCKed by caller
//
...
//    return -1;
//
//  debug("cReelTextEffects::DrawAnimatedTitle(%d, %d, %s)", o_id, ReelConfig.useTextEffects, s);
//
//  if (o_id >= 0) {
...
//  } else {
//    skin->DrawTitle(s);
//    const cFont *Font = ReelConfig.GetFont(FONT_OSDTITLE);
//    if (ReelConfig.useTextEffects && ((Font->Width(s ? s : "") > Width) || (action > 0))) {
//      // New scrolling text
...
//}
//
//int cReelTextEffects::DrawAnimatedText(int o_id, int action, int x, int y, const char *s, tColor ColorFg, tColor ColorBg, int FontId, int Width, int Height, int Alignment)
//{
//  //Must be TE_LOCKed by caller
//
//  if (osd == NULL)
//    return -1;
//
//  debug("cReelTextEffects::DrawAnimatedText(%d, %d, %s)", o_id, ReelConfig.useTextEffects, s);
//
//  if (o_id >= 0) {
//    // Update animated text
//    tEffect *effect = vecEffects[o_id];
...
//    }
//  } else {
//    const cFont *Font = ReelConfig.GetFont(FontId);
//    if (Height == 0)
...
#ifdef HAVE_FREETYPE
const char **cReelTextEffects::GetAvailTTFs(void)
{
  if (availTTFs == NULL) {
    std::vector<std::string> vecFonts;
    cReadDir d(ReelConfig.GetFontsDir());
    struct dirent *e;
    while ((e = d.Next()) != NULL) {
      if ((strcmp(e->d_name, ".") != 0) && (strcmp(e->d_name, "..") != 0)) {
          if (strcmp(e->d_name + strlen(e->d_name) - 4, ".ttf") == 0   || strcmp(e->d_name + strlen(e->d_name) - 4, ".otf") == 0) { // show .otf as well!
