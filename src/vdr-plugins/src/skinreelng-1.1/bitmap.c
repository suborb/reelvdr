...
bool cOSDImageBitmap::DrawMagick(const char *Filename, int x, int y, int width, int height, int colors, cBitmap *bmp)
{
  ...
  try {
    ...
    int h = geo.height();
    if (height != h || width != w) {
      switch (ReelConfig.resizeImages) {
        case 0:
    ...
    esyslog("skinreelng: Couldn't load %s: %s", Filename, e.what());
    return false;
  } catch (...) {
    esyslog("skinreelng: Couldn't load %s: Unknown exception caught", Filename);
    return false;
  }
  return true;
}
