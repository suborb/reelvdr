...
//#endif
  ...
  // this might be a cleaner implementation
    if (::Setup.FontSizes == FONT_SIZE_NORMAL) // normal fonts
      ...
      if (::Setup.FontSizes == FONT_SIZE_LARGE) // Large fonts
        for (i=0; i < FONT_NUMFONTS; i++)
        {
                strcpy(allFonts[i].Name, DEFAULT_TTF_NAME); //Name
                allFonts[i].VdrId = FONT_TRUETYPE;
        ...
        if (::Setup.FontSizes == FONT_SIZE_SMALL) // SmallFonts
          for (i=0; i < FONT_NUMFONTS; i++)
          {
                strcpy(allFonts[i].Name, DEFAULT_TTF_NAME); //Name
                allFonts[i].VdrId = FONT_TRUETYPE;
      ...
      if (::Setup.FontSizes != FONT_SIZE_USER) // not userdefined 
      {
