...
bool cReelLogoCache::DrawEventImage(const cEvent *Event, int x, int y, int w, int h, int c, cBitmap *bmp)
{
  ...
  asprintf(&strFilename, "%s/%d.%s", ReelConfig.GetImagesDir(), Event->EventID(), ReelConfig.GetImageExtension());
...
bool cReelLogoCache::DrawRecordingImage(const cRecording *Recording, int x, int y, int w, int h, int c, cBitmap *bmp)
{
  ...
  char *strFilename = NULL;
  asprintf(&strFilename, "%s/%s.%s", Recording->FileName(), RECORDING_COVER, ReelConfig.GetImageExtension());
  int rc = DrawImage(strFilename, x, y, w, h, c, bmp);
  free (strFilename);
  return rc;
}
...
bool cReelLogoCache::Load(const char *fileNameP, int w, int h, bool fLogNotFound)
{
  ...
  char *strFilename;
  asprintf(&strFilename, "%s/%s.xpm", ReelConfig.GetLogoDir(), fileNameP);
  if (strFilename == NULL)
    return false;

  debug("cPluginSkinReel::Load(%s)", strFilename);
  ...
  if (i != cacheMapM.end()) {
    ...
    if (cacheSizeM) {
      ...
      debug("cPluginSkinReel::Load() INSERT(%s)", strFilename);
