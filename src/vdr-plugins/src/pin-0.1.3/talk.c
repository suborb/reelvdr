...
Talk::~Talk()
{
   if (opened)
   {
      if (msgctl(msgID, IPC_RMID, 0) < 0)
         syslog(LOG_ERR,"Error: Closing message queue failed, '%s'\n", strerror(errno));
   }
}
...
int Talk::open(long port)
{
   ...
   {
      syslog(LOG_ERR,"Creating of message queue failed, error was '%s'\n", strerror(errno));
   ...
   if ((result = msgrcv(msgID, theMsg, sizeBuffer, 0, 0)) < 0)
   {
      if (!fTimeout)
      {
         if (errno == EINTR)
            ...
            syslog(LOG_ERR,"Receiving message failed, errno (%d), '%s'", 
...
int Talk::send(long to, int event, const char* msg)
{
   ...
   {
      syslog(LOG_ERR,"Message to big, aborting\n");
      return fail;
   }
   ...
   {
      syslog(LOG_ERR, "Attach to message queue failed, '%s'\n", strerror(errno));
      return fail;
   }
   ...
   syslog(LOG_DEBUG,"send to (%ld) succeeded\n", header->receiver);
