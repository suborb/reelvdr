...
*/
            ...
            if (cRemote::CallPlugin("xinemediaplayer"))
                printf("(%s:%d) Called xinemediaplayer\n",__FILE__, __LINE__);
            return true;
#endif
            ...
            if (cXineOsd::isPlaylist && cXineOsd::playListEntries.size() > 0)
            {
                ...
                savedPlaylist.Save();
            } else if (cXineOsd::isPlaylist && cXineOsd::playListEntries.size()==0)
            {
            ...
            if (!cXineOsd::isPlaylist && cXineOsd::playListEntries.size() <= 0  && cXineOsd::mrl!= NULL) {
		...
		printf("\033[0;91mmrl = %s\033[0m\n", cXineOsd::mrl);
            ...
            if (savedPlaylist.playlist.size() > 0)
            {
                ...
                if(cXineOsd::isPlaying)
                {
                    cXineOsd::control->SetPlayList(savedPlaylist.playlist);
                    cXineOsd::firstOpen = false;
                }
                else
                {
                    cXineOsd::control = new Control(
                                cXineOsd::playListEntries.at(0).c_str(),
                                1, // always a playlist
                                cXineOsd::playListEntries
                                );
            ...
            if (getSetPlaylist->set) {
                ...
                getSetPlaylist->playlistEntries = cXineOsd::playListEntries;
