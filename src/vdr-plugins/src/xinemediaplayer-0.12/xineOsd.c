...
#if APIVERSNUM < 10700
        cXineOsd::cXineOsd() : cOsdMenu("Mediaplayer",  7, 15)
#else
        cXineOsd::cXineOsd() : cOsdMenu("Mediaplayer",  8, 15)
#endif
        {
            //LOC; printf( "%s\n", mrl );
                        //DEBUG_VAR(mrl);
            ...
            if ( playListEntries.size()<= 0 && mrl != NULL )
            {
...
//          printf("got player position %i\n", currentlyPlaying);
            ...
            if (prevCurrent != currentlyPlaying ) // update title
            {
                ...
                if (isShowingCover)
                    setTitle_str = "SHOWINGMP3COVER$Mediaplayer";
                else
                    setTitle_str = "Mediaplayer";
                if(currentlySelected >= 0 && currentlySelected < (int)playListEntries.size())
                {
                    int success = Id3Info(playListEntries[currentlySelected], title, artist, album, comment, genre, &year, &track);
        ...
        void cXineOsd::ShowEntries()
        {
            ...
            if ( currentlyPlaying < currentlySelected )
            ...
            if (currentlySelected >= 0 &&
                ...
                Id3Info( playListEntries[currentlySelected],
        ...
        void cXineOsd::ShowReplayModeInSideNote()
        {
            ...
            SideNote(&replayModes);
                 ...
                 * else it is not cleared when playing internet radio*/
                 ...
                 if ( !CanHaveDifferentModes() ) return;
                  ...
                  switch( _mode )
                  {
                    char status_str[18];
                    case PLAY_NORMAL:
                      snprintf(status_str, sizeof(status_str), "%s: %s", tr("Mode"), tr("Normal") );
                      SetStatus(status_str);
                      break;

                    //case PLAY_REPEAT_SINGLE:
                    case PLAY_REPEAT_PLAYLIST:
                      snprintf(status_str, sizeof(status_str), "%s: %s", tr("Mode"), tr("Repeat") );
                      SetStatus(status_str);
                      break;

                    case PLAY_RANDOM:
                      snprintf(status_str, sizeof(status_str), "%s: %s", tr("Mode"), tr("Shuffle") );
                      SetStatus(status_str);
                      break;
                ...
                {
                    //printf("Xine: CallFilebrowser, browserInstance = %d\n", cXineOsd::browserInstance);
...
//          printf("xineosd::ProcessKey(%i)\n",Key);
            eOSState state = osContinue;
                        ...
                        if (lastVolShownTime > 0 && time(0) - lastVolShownTime >= 2) // clear
                        {
                            SetStatus(NULL); // clear volume status
                            lastVolShownTime = 0;
                            ShowPlayMode(true);
                        }
                        else if ( time(0) - lastVolShownTime <=1 )
                        {
                            if (!cDevice::PrimaryDevice()->IsMute())
                            {
                                std::string vol;
                                vol = tr("Volume") + std::string(": ") + MakeProgressBarStr( cDevice::PrimaryDevice()->CurrentVolume()*100.0/255, 30  );
                                SetStatus(vol.c_str() );
                                lastVolShownTime = time(0);
                            }
                            //donot respond to mute: Done in ShowPlayMode()
                        }

                        currentlySelected = Current();
                        if (currentlySelected > currentlyPlaying )
...
// Directy quit                     cRemote::Put(kBack);
                    ...
                    if (!control->PlayFileNumber( currentlySelected ) )
            ...
            static time_t lastUserInputTime = time(0);
            if (Key != kNone )
                lastUserInputTime = time(0);
            else // Key == kNone
                if (time(0) - lastUserInputTime > 7 ) // return to current file
        ...
        eOSState cXineOsd::ShowCover(bool hide)
        {
            ...
            else if(dirty)
            {
                ...
                if(cache && cache->FetchFromCache(cover, thumb, coverSize, thumbConvert->GetEnding()))
                {
                    ...
                    std::string title, dummy;
                    std::string setTitle_str = "SHOWINGMP3COVER$Mediaplayer";
