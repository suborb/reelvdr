...
struct PageID {
   ...
   PageID(const cChannel* id, int p, int s) { set(id, p, s); }
   void set(const cChannel* id, int p, int s)
...
public:
   ...
   void prepareDirectory(const cChannel* chan);
...
#else
   cTxtReceiver(const cChannel* chan);
#endif
   ~cTxtReceiver();
};
