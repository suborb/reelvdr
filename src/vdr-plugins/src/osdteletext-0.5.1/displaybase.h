...
public:
    ...
    // Map this teletext color to an OSD color in #Area.
    // Defaults to 1:1 identity
    ...
    // return same as GetColorIndex(). Used to solve conflicts if
...
protected:
    ...
    static unsigned int* GetFontChar(cTeletextChar c, unsigned int *buffer);
...
public: 
    void RenderTeletextCode(unsigned char *PageCode) {
        ...
        cRenderPage::RenderTeletextCode(PageCode);
