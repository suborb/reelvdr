...
static const char *VERSION        = "0.5.1-reel";
...
bool cPluginTeletextosd::Start(void)
{
  // Start any background activities the plugin shall perform.
   ...
   if (ttSetup.OSDwidth<=100)	ttSetup.OSDwidth=Setup.OSDWidth * 0.75;
...
{
    if (Id && strcmp(Id, "StopReceiver") == 0) {
        txtStatus->CheckDeleteReceiver();
        return true;
    } else if (Id && strcmp(Id, "StartReceiver") == 0) {
        txtStatus->CheckCreateReceiver();
        return true;
    } 

#if VDRVERSNUM >= 10716 && REELVDR
     else if(Id && strcmp(Id, "ActiveModeChanged") == 0) {
        if(Data) {
