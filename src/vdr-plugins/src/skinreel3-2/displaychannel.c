...
void cSkinReelDisplayChannel::DrawChannelLogo(const cChannel *channel) {
 ...
 std::string logoPath = ReelConfig.GetLogoPath(channel->Name());
...
// clear signal icon/"Tuner X.Y" icon
osd->DrawImage(imgMenuFooterCenterX, xsSignalBar, yBottomTop, false, 90,1);
...
#else
 {
     // show Tuner_X_Y.png according to status 
     // where X: slot number and Y: tuner number/subslot
          int number  = ((status >> 12))&7;
          int slot    = 1 + ((number-1) >> 1);
          int subslot = 1 + ((number&1)^1);

          if (number && 
              slot > 0     && slot    <= 3 /* max. 3 slots */ && 
              subslot > 0  && subslot <= 2 /* max. tuners in one slot := 2 */
              )
          {
              char tuner_img[16]; // Tuner_1_2.png
              snprintf(tuner_img, 15, "Tuner_%d_%d.png", slot, subslot);

              DrawUnbufferedImage(osd, tuner_img , xsSignalBar,
                      y - CHANNELINFOSYMBOLHEIGHT/2, true);

          }
          else // status not available, indicate by drawing "Tuner X.X" icon
              DrawUnbufferedImage(osd, "Tuner_x_x.png" , xsSignalBar,
                      y - CHANNELINFOSYMBOLHEIGHT/2, true);
 }
#endif
 xsSignalBar += 90 + SmallGap;  // 72 is imgSignal length
...
void cSkinReelDisplayChannel::UpdateAspect() {
  ...
  if (h == 1080 || h == 720) // HD video
      osd->DrawImage(imgIconHd, xSymbol, ys, true);
  else if(4.0/3.0 == lastAspect)
      osd->DrawImage(img43, xSymbol, ys, true);
