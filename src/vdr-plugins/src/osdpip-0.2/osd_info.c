...
#define DRAW_PIP_FRAME(bm,x,y) { \
	if(cPluginManager::GetPlugin("reelice")) { \
		int color = Skins.Current()->Theme()->Color("clrBackground"); \
		for(int i=5; i>=-1; i--) \
			bm->DrawRectangle(x-i, y-i, x+190+i, y+150+i, (i>=0) ? ((color&0xFFFFFF)|0xF000000|(0xF-i*2)<<28) : clrBlack/*clrTransparent*/); \
	}}
#define DRAW_TXT_FRAME(bm,x,y,w,h) { \
	if(cPluginManager::GetPlugin("reelice")) { \
		int color = Skins.Current()->Theme()->Color("clrBackground"); \
		bm->DrawRectangle(x-5, y-5, x+190+5, y+h+5, ((color&0xFFFFFF)|0x8F000000)); \
	}}
...
void cOsdInfoWindow::Show(bool pipChannel, bool dummy)
{
    ...
    DRAW_PIP_FRAME(m_Bitmap,m_pipPosX,m_pipPosY);
...
#define DATEW 60
    ...
    if(!dummy){
     if(pipChannel) {
      if(!atBottom) ty += cPluginManager::GetPlugin("reelice") ? 30 : 60;
      ty -= 60;
      if(atBottom && OsdPipSetup.ShowInfo == 2)
        ty += 30;
      else if (atBottom && OsdPipSetup.ShowInfo == 1)
        ty += 60;
      DRAW_TXT_FRAME(m_Bitmap, m_x, ty, textw+DATEW, font->Height());
