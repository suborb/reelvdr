/*
 * status.h: 'ReelNG' skin for the Video Disk Recorder
 *
...
#define TE_START(osd) ReelTextEffects.Start(osd);
#define TE_STOP ReelTextEffects.Stop();
#define TE_LOCK ReelTextEffects.UpdateLock();
#define TE_UNLOCK ReelTextEffects.UpdateUnlock();
#define TE_WAKEUP ReelTextEffects.RefreshEffects();
#define TE_MARQUEE(osd, id, x...) ReelTextEffects.DrawAnimatedText(id, 0, x)
#define TE_BLINK(osd, id, x...) ReelTextEffects.DrawAnimatedText(id, 1, x)
#define TE_TITLE(osd, id, x...) ReelTextEffects.DrawAnimatedTitle(id, 0, x)

#endif //DISABLE_ANIMATED_TEXT

//#ifdef DISABLE_ANIMATED_TEXT
class cReelTextEffects {
//#else
//class cReelTextEffects : public cThread {
//#endif
private:
  cOsd *osd;
...
public:
//#ifdef DISABLE_ANIMATED_TEXT
  cReelTextEffects(void);
  ~cReelTextEffects(void);

//#else
//  cReelTextEffects(const char *Description = NULL);
//  ~cReelTextEffects(void);
//
...
//  int DrawAnimatedTitle(int o_id, int action, const char *s, int Width, cSkinReelThreadedOsd *skin);
...
extern cReelTextEffects ReelTextEffects;
