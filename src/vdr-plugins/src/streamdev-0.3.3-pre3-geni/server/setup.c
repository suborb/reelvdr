...
//	AddSuspEdit (tr("Suspend behaviour"),         m_NewSetup.SuspendMode);
	Add(new cMenuEditStraItem(tr("Suspend behaviour"), &m_NewSetup.SuspendMode, sm_Count, SuspendModesTR));
	AddBoolEdit (tr("Client may suspend"),        m_NewSetup.AllowSuspend);
	
	AddCategory (tr("ReelBox-to-ReelBox Server"));
	AddBoolEdit (tr("Start ReelBox-to-ReelBox Server"),   m_NewSetup.StartVTPServer);
	AddShortEdit(tr("ReelBox-to-ReelBox Server Port"),    m_NewSetup.VTPServerPort);
