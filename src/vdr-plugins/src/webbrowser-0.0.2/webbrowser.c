...
static const char *MAINMENUENTRY = "Web Browser";
...
void cMenuWebbrowserSetup::Setup(void)
{
    ...
    Clear();
    if (!HasMode(MODE_LINKS) && !HasMode(MODE_FIREFOX) && !HasMode(MODE_X11))
    {
    ...
    Add(new cMenuEditModeItem(tr("Mode"), &WebbrowserSetup.mode, 3, modeStrings));
    ...
    {
        if (WebbrowserSetup.mode == MODE_FIREFOX)
            Add(new
                cMenuEditStrItem(tr("start URL"), WebbrowserSetup.FFstartURL, 255,
                                 allowed));
        Add(new
            cMenuEditIntPercItem(tr("transparency"), &WebbrowserSetup.X11alpha, 25, 255,
                                 NULL, NULL, 5));
        Add(new
            cMenuEditStraItem(tr("scaling mode"), &WebbrowserSetup.center_full, 2,
                              centerFullStrings));
    }
