...
void cPluginSkinReelSetup::Store(void)
{
 PRINT_FUNC;
  ...
  SetupStore("ShowSourceInChannelinfo", ReelConfig.showSourceInChannelinfo);
  SetupStore("ProgressBarType", ReelConfig.pgBarType);
...
cMenuSetupGeneral::cMenuSetupGeneral(cReelConfig* Data) : cMenuSetupSubMenu(tr("General"), Data)
{
  ...
  pgbarTexts[0] = tr("simple");
  pgbarTexts[1] = tr("graphical");
  pgbarTexts[2] = tr("graphical small");
...
void cMenuSetupGeneral::Set(void) {
  ...
  Add(new cMenuEditBoolItem(tr("Show source in the channelinfo"), &data->showSourceInChannelinfo));
  Add(new cMenuEditStraItem(tr("Progressbar type"), &data->pgBarType, 3, pgbarTexts));
...
void cMenuSetupGeneral::Store(void) {
#ifndef SETUP_ALL
  ReelConfig = *data;
  SetupStore("SingleArea8Bpp", ReelConfig.singleArea8Bpp);
  SetupStore("ShowAuxInfo", ReelConfig.showAuxInfo);
...
eOSState cMenuSetupLogos::ProcessKey(eKeys Key) {
  ...
  if (Key != kNone && ((data->showLogo != oldShowLogo) || (data->showSymbols != oldShowSymbols) 
       || (data->showStatusSymbols != oldShowStatusSymbols) || (data->showSymbolsMenu != oldShowSymbolsMenu) 
