		...
		case build:
			{
				if (line.substr(0, px_found_id.length()) == px_found_id) {
					...
					set_progress(static_cast<int>(done));
                                        NotifyBgProcess(tr("Burning DVD"), get_job().get_title().c_str(), start_time, static_cast<int>(done)); // to BgProcess List
				}
		...
		case burn:
			{
				...
				if (rest.get() == burn_done) {
					...
					set_burn_progress(static_cast<int>(percentDone));
                               ...
                               NotifyBgProcess(tr("Burning DVD"), get_job().get_title().c_str(), start_time, static_cast<int>(percentDone)); // to BgProcess List
			}
			break;
		};
	}
		...
		if (proc->return_status() == process::ok) {
			...
			if (proc->name() == "demux" && global_setup().DemuxType == demuxtype_projectx)
				...
				execute(eject);
                        ...
                        NotifyBgProcess(tr("Burning DVD"), tr("done"), start_time, 101); // to remove from BgProcess List
			return true;
		}
	...
	bool chain_dvd::prepare_cutmarks()
	{
...
#if APIVERSNUM >= 10700
                cIndexFile ifile( m_currentRecording->get_filename().c_str(), false, m_currentRecording->isTS() );
#else
		...
		for (cMark *mark = marks.First(); mark != 0; mark = marks.Next(mark)) {
			...
			int pos = mark->position;
                        ...
                        bool fl;
                        int i;
                        ifile.Get(pos, &filenumber, &offset, &fl, &i);
                        boost::format filefmt( "%s/%03d.vdr" );
                        if(m_currentRecording->isTS())
                            filefmt =  boost::format("%s/%05d.ts") ;
