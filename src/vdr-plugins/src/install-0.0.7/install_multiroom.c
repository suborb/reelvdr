...
void cInstallMultiroom::ConfigureAsStandalone()
{
    ...
    // client --> standalone
    // remove MEDIADEVICE if it is non-local
    if (Setup.ReelboxMode == eModeClient)
    {
        const char* mediadev = sysconfig.GetVariable("MEDIADEVICE");

        // non-local MEDIADEVICE
        if (mediadev && strstr(mediadev,"/mount/.mount"))
        {
            // Use local harddisk
            ...
            isyslog("MEDIADEVICE changed '%s' --> '%s'", mediadev, UUID.c_str());
        } // non-local mediadev
    } // client --> standalone
