...
public:
  ...
  int ReadJpeg(char *sPath);
  int DumpRaw(char *sPath, int step=1);

  int WriteFile(unsigned char nTransparenz, int xpos, int ypos, char *sPath, bool bHasTransparency=false, int rel_width=0, int rel_height=0, int rel_rowsize=0);
...
//  bool DrawRawBitmap(cOsd *Osd, char *path, int xpos, int ypos, int maxheight=0);
  ...
  bool ReadPng(char *path);
  bool ReadRawBitmap(char *path);
  bool PngIsReadable(char *path);
