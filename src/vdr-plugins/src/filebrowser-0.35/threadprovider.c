...
: state_(stIdle),parallel_(true), thread_(NULL), ret_(0) 
{
}
...
cInstallDebPackTask::cInstallDebPackTask(const std::string &file)
: file_(file)
{
    printf("---------------cInstallDebPackTask::cInstallDebPackTask------------\n");
}

cWorkThread *cInstallDebPackTask::NewThread(cThreadProvider *provider)
{
    thread_ = new cInstallDebPackThread(this, provider);
    return thread_; 
}

void cInstallDebPackTask::ProcessTask()
{  
    printf("---------------cInstallDebPackTask::ProcessTask()------------\n");
  
    //std::string message;
    //message = tr("please wait ...");
    //Skins.Message(mtInfo,message.c_str());
#ifndef RBLITE
    std::string cmd = std::string("dpkg -i ") + file_;
#else
    std::string cmd = std::string("ipkg-cl install ") + file_;
#endif
    ret_ = ::SystemExec(cmd.c_str());
    /*if(ret_ == 0)
    {
...
cInstallDebPackThread::cInstallDebPackThread(cInstallDebPackTask *task, cThreadProvider *provider) 
: cWorkThread(task, provider)
{
    SetPriority(10);
}
...
cInstallDebPackTask *cInstallDebPackThread::GetInstallDebPackTask()
{
    return static_cast<cInstallDebPackTask*>(task_);
}
